...
# Anchor confirmed in previous test
...
class App(EWrapper, EClient):
    ...
    def error(
        ...
        # Normal farm/status messages
        if errorCode in (2104, 2106, 2158):

            return

        # 10090 is useful but extremely repetitive
        if errorCode == 10090:
...
def display(app, contracts, request_map, expiry):
    ...
    print(
        f"New York : {now_ny().strftime('%Y-%m-%d %H:%M:%S')}"
    )
...
def main():
    ...
    print(
        f"Contracts found: {len(contracts)}/{len(strikes) * 2}"
    )
    ...
    print()
    print(
        f"L1 subscriptions started: {len(request_map)}"
    )
